...
目标供应商能力以官方模型目录与协议文档为基准：OpenAI GPT-6 Astra、Sol、Luna，以及 DeepSeek `deepseek-flash` 和官方列出的 V4 兼容别名支持原生图片输入。OpenAI Chat Completions 的图片能力不等价于 Agent 工具调用能力；GPT-6 Astra 的工具请求需要 Responses，Sol/Luna 当前 Chat 工具调用要求显式 `reasoning_effort=none`。OpenAI [模型目录](https://developers.openai.com/api/docs/models)、[最新模型指南](https://developers.openai.com/api/docs/guides/latest-model)和 DeepSeek [更新日志](https://api-docs.deepseek.com/updates/)、[Vision 指南](https://api-docs.deepseek.com/guides/vision/)是这些精确档案的事实来源。

## 决策

- 模型图片能力由 provider、当前实际协议、model ID 三元组解析。精确官方渠道档案、同渠道明确返回的 `input_modalities` 和管理员精确覆盖按优先级合并；无证据时为 `unknown`。能力与来源写入现有 `ModelInfo`/Redis 视图，并由 `/models/v2` 同时提供给 API、Worker 和 Web，不新建能力表或第二模型目录。
...
| 验收主张 | 直接证据 | 负向证据 | 当前结果 |
|---|---|---|---|
| 能力按渠道、协议、模型精确解析并保留出处 | resolver、cache、provider service 与 `/models/v2` 单测 | 同名跨渠道/协议不命中；未知不升级为支持 | 通过：相关 136 个后端 unit 全部通过 |
...
| Web 上传 MIME 与后端能力展示保持一致 | MIME unit、lint、unit、production build；关键控件源码由组件测试覆盖 | PNG 不得被改写为 JPEG；前端不覆盖后端 profile | 逻辑验证通过；当前浏览器未登录，本次未补登录后页面截图 |
