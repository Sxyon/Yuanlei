"""专属沙盒管理 HTTP 表面：列表、配额用量、手动预热、回收与重建。"""
...
def _management_error(exc: Exception) -> HTTPException:
    ...
    logger.warning("sandbox management operation failed: %s", exc, exc_info=exc)
...
@coding_sandboxes.post("/{agent_slug}/{project_id}/provision")
async def provision_coding_sandbox(
    agent_slug: str,
    project_id: str,
    current_user: User = Depends(get_required_user),
    db: AsyncSession = Depends(get_db),
    ...
    try:
        return await SandboxManagementService(db).provision(
