...
@pytest.mark.parametrize("connection_error", [httpcore.ConnectError, httpcore.ConnectTimeout])
async def test_backend_falls_back_after_httpcore_connection_error(monkeypatch, connection_error):
    """真实 httpcore 建连异常也必须继续尝试已校验的下一个公网地址。"""
    _patch_getaddrinfo(monkeypatch, _addr_infos("93.184.216.34", "8.8.8.8"))
    ...
    class FailingBackend(RecordingBackend):
        async def connect_tcp(self, host, port, timeout=None, local_address=None, socket_options=None):
            ...
            if host == "93.184.216.34":
                raise connection_error("unreachable")
            return object()

    recording = FailingBackend()
    await SSRFGuardBackend(default_backend=recording).connect_tcp("example.com", 443)

    assert [host for host, _, _ in recording.calls] == ["93.184.216.34", "8.8.8.8"]
