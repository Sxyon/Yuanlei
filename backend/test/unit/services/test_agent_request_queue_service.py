...
@pytest.mark.asyncio
async def test_persist_request_passes_project_override_to_config_resolution(session, monkeypatch: pytest.MonkeyPatch):
    """项目数字员工的 model/审批覆盖进入请求 input_payload。"""
    from yuxi.services.input_message_service import build_chat_input_message

    captured: list[dict | None] = []

    async def resolve_config(model_spec, tool_approval_mode, agent_item, agent_backend, db, project_override=None):
        del model_spec, tool_approval_mode, agent_item, agent_backend, db
        captured.append(project_override)
        return "provider:project-model", "default"

    async def project_override(*, db, agent_slug, project_id):
        del db, agent_slug, project_id
        return {"model": "provider:project-model"}

    monkeypatch.setattr(agent_request_service, "resolve_agent_run_config", resolve_config)
    monkeypatch.setattr(agent_request_service, "load_project_agent_override", project_override)
    await _seed_thread(session)
    ...
    result, _ = await _persist_request(
        ...
        agent_backend=MagicMock(),
        request_input=AgentRequestInput(
            request_id="request-project-override",
            agent_slug="main",
            thread_id="t1",
            input_message=build_chat_input_message("override"),
            origin=RunOrigin(source="chat", channel="web"),
        ),
        current_user=SimpleNamespace(uid="user-1"),
    )

    assert result.input_payload["model_spec"] == "provider:project-model"
    assert captured == [{"model": "provider:project-model"}]
