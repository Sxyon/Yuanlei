...
async def _resolve_project_scope(*, db, run_id: str, uid: str) -> tuple[str, User]:
...
async def _run_dashboard_operation(
    ...
    try:
        ...
        async with pg_manager.get_async_session_context() as db:
            project_id, user = await _resolve_project_scope(db=db, run_id=run_id, uid=uid)
...
@tool(category="buildin", tags=["项目"], display_name="读取项目 Dashboard")
async def dashboard_read(runtime: ToolRuntime = None) -> str:
    """读取当前 Project 的 Dashboard 页面状态、revision、hash 与 HTML。
...
@tool(category="buildin", tags=["项目"], display_name="更新项目 Dashboard")
async def dashboard_write(html: str, expected_revision: int, runtime: ToolRuntime = None) -> str:
    """用完整 HTML 创建或更新当前 Project 的 Dashboard 入口页面。

    仅在用户明确要求生成或修改项目 Dashboard 时使用。必须携带提交前读到的
    revision；返回 revision_conflict 时先调用 dashboard_read 读取最新页面，
    合并用户要求后重新提交，不能自动重试旧内容。
    """
