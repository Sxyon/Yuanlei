...
async def submit_agent_request(
    *,
    ...
    if existing_request:
        ...
        await _require_agent_project_scope(db=db, agent_slug=agent_item.slug, project_id=conversation.project_id)
...
async def _require_agent_project_scope(*, db: AsyncSession, agent_slug: str, project_id: str | None) -> None:
    """把项目数字员工的运行范围拒绝收敛为稳定 HTTP 语义。"""
    try:
        await ensure_agent_project_scope(db=db, agent_slug=agent_slug, project_id=project_id)
